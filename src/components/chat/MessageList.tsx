...
function getToolLabel(toolName: string, args: any, done: boolean): string {
  const path: string = args?.path ?? "";
  const cmd: string = args?.command ?? "";

  if (toolName === "str_replace_editor") {
    switch (cmd) {
      case "create":      return done ? `Created ${path}`          : `Creating ${path}`;
      case "str_replace": return done ? `Edited ${path}`           : `Editing ${path}`;
      case "insert":      return done ? `Inserted into ${path}`    : `Inserting into ${path}`;
      case "view":        return done ? `Viewed ${path}`           : `Viewing ${path}`;
      case "undo_edit":   return done ? `Undid edit on ${path}`    : `Undoing edit on ${path}`;
    }
  }
  ...
  if (toolName === "file_manager") {
    ...
    switch (cmd) {
      case "rename": return done ? `Renamed ${path} → ${newPath}` : `Renaming ${path}`;
      case "delete": return done ? `Deleted ${path}`               : `Deleting ${path}`;
    }
  }
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation": {
                            ...
                            const label = getToolLabel(tool.toolName, tool.args, !!done);
                            return (
